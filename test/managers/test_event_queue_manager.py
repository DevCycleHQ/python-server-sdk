...
from devcycle_python_sdk.models.event import FlushPayload, UserEventsBatchRecord, RequestEvent
...
class EventQueueManagerTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.test_options = DevCycleLocalOptions(disable_automatic_event_logging=False,
                                                 disable_custom_event_logging=False)
        self.test_options_no_thread = DevCycleLocalOptions(disable_automatic_event_logging=True,
    ...
    def test_init(self):
        ...
        manager = EventQueueManager(self.sdk_key, self.test_options, self.test_local_bucketing)
    ...
    def test_publish_event_payload_retryable_api_error(self, mock_publish_events):
        ...
        self.test_local_bucketing.on_event_payload_failure.assert_called_with(self.test_payload.payloadId, True)
    ...
    def test_publish_event_payload_non_retryable_api_error(self, mock_publish_events):
        mock_publish_events.side_effect = APIClientUnauthorizedError("Some retryable error")
        ...
        # should try and publish events, get the error and then report the failure to local bucketing
        mock_publish_events.assert_called_once()
        self.test_local_bucketing.on_event_payload_failure.assert_called_with(self.test_payload.payloadId, False)
        self.test_local_bucketing.on_event_payload_success.assert_not_called()
